...
class ShapeSpaceRunner:
    ...
    def run(self):
        ...
        for epoch in range(self.startepoch, self.nepochs + 1):
            ...
            for data_index, (mnfld_pnts, normals, indices) in enumerate(self.train_dataloader):
                ...
                nonmnfld_with_bias, nonmnfld_wb_gt = self.sampler.get_points(mnfld_pnts, with_bias=True)

                nonmnfld_wb_gt = nonmnfld_wb_gt.reshape(mnfld_pnts.shape[0] * mnfld_pnts.shape[1], -1)

                mnfld_pnts = self.add_latent(mnfld_pnts, indices)
                nonmnfld_pnts = self.add_latent(nonmnfld_pnts, indices)
                nonmnfld_with_bias = self.add_latent(nonmnfld_with_bias, indices)

                # forward pass
                mnfld_pnts.requires_grad_()
                nonmnfld_pnts.requires_grad_()
                nonmnfld_with_bias.requires_grad_()

                mnfld_pred = self.network(mnfld_pnts)
                nonmnfld_pred = self.network(nonmnfld_pnts)
                nonmnfld_wb_pred = self.network(nonmnfld_with_bias)

                mnfld_grad = gradient(mnfld_pnts, mnfld_pred)
                nonmnfld_grad = gradient(nonmnfld_pnts, nonmnfld_pred)
                nonmnfld_wb_grad = gradient(nonmnfld_with_bias, nonmnfld_wb_pred)

                # manifold loss
                mnfld_loss = (mnfld_pred.abs()).mean()
                wb_loss = ((nonmnfld_wb_pred - nonmnfld_wb_gt).abs()).mean()

                # partial z norm loss
                grad_loss_1 = ((nonmnfld_grad[:, -1] - 1) ** 2).mean()
                grad_loss_2 = ((nonmnfld_wb_grad[:, -1] - 1) ** 2).mean()
                grad_loss = grad_loss_1 + grad_loss_2

                loss = mnfld_loss + self.grad_lambda * grad_loss + wb_loss
    ...
    def __init__(self, **kwargs):
        ...
        self.sampler = Sampler.get_sampler(self.conf.get_string('network.sampler.sampler_type'))(
        ...
        self.train_dataloader = torch.utils.data.DataLoader(
            self.ds, batch_size=self.batch_size, shuffle=True,
            num_workers=kwargs['threads'], drop_last=True, pin_memory=True)
        self.eval_dataloader = torch.utils.data.DataLoader(
            self.ds, batch_size=1, shuffle=True,
            num_workers=1, drop_last=True)

        self.network = utils.get_class(self.conf.get_string('train.network_class'))(
            d_in=(self.d_in + self.latent_size), **self.conf.get_config('network.inputs'))
        ...
        if is_continue:
            ...
            data = torch.load(os.path.join(old_checkpnts_dir, self.latent_codes_subdir,
                                           str(kwargs['checkpoint']) + '.pth'), map_location="cpu")
            self.lat_vecs = data["latent_codes"].cuda()

            saved_model_state = torch.load(os.path.join(old_checkpnts_dir, 'ModelParameters',
                                                        str(kwargs['checkpoint']) + ".pth"), map_location="cpu")
            # Handle both DDP-wrapped (module. prefix) and plain checkpoints
            state_dict = saved_model_state["model_state_dict"]
            state_dict = {k.replace('module.', ''): v for k, v in state_dict.items()}
            self.network.load_state_dict(state_dict)

            data = torch.load(os.path.join(old_checkpnts_dir, 'OptimizerParameters',
                                           str(kwargs['checkpoint']) + ".pth"), map_location="cpu")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--points_batch', type=int, default=4096, help='point batch size')
    parser.add_argument('--nepoch', type=int, default=40000, help='number of epochs to train for')
    ...
    parser.add_argument('--threads', type=int, default=0, help='num of threads for data loader')
    parser.add_argument('--is_continue', default=False, action="store_true", help='continue')
    parser.add_argument('--timestamp', default='latest', type=str)
    parser.add_argument('--checkpoint', default='latest', type=str)
    parser.add_argument('--split', default='dfaust/2022_train.json', type=str)
