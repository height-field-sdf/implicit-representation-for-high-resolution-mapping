...
def optimize_latent(points, normals, conf, num_of_iterations, network, lr=1.0e-2):
    ...
    normals_lambda = conf.get_float('network.loss.normals_lambda')# if without normals, =0
...
######### only rectify grad_loss
        surface_loss = torch.abs(surface_pred).mean()
        # partial z norm loss
        grad_loss = torch.mean((nonsurface_grad[:,-1]-1).pow(2))+ torch.mean((surface_grad[:,-1]-1).pow(2))

        # partial z abs loss
        # grad_loss = torch.mean((nonsurface_grad[:,-1]-1).abs())+ torch.mean((surface_grad[:,-1]-1).abs())
        normals_loss = ((surface_grad - normals).abs()).norm(2, dim=1).mean()
        latent_loss = latent.abs().mean()
        loss = surface_loss + latent_lambda * latent_loss + normals_lambda * normals_loss + grad_lambda * grad_loss
...
def optimize_latent_point(points, normals, conf, num_of_iterations, network, lr=1.0e-2):
    ...
    num_of_points, dim = points.shape
    print(f'num_of_points, dim are:{num_of_points},{dim}')
    # sys.exit(0)
...
def optimize_latent_ani(points, normals, conf, num_of_iterations, network, lr=1.0e-2):
    ...
    latent_ls = []
    latent_ls.append(latent.clone().detach().unsqueeze(0))
    ...
    for i in range(num_of_iterations):
        ...
        # print(f'latent content:{latent[:10]}')
        # # print(f'latent shape:{latent.unsqueeze(0).shape}')
        # print(f'latent ls length:{len(latent_ls)},shape:{latent_ls[i].shape}, content: {latent_ls[i][0,:3]}')
        
        # if i > 1:
        #     sys.exit(0)
        
    return latent.unsqueeze(0),latent_ls
