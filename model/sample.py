...
class NormalPerPoint(Sampler):
    ...
    def get_points(self, pc_input, local_sigma=None, with_bias=False):
        batch_size, sample_size, dim = pc_input.shape
        ...
        if with_bias:
            # maxT, minT = 0.001, -0.001 # 之前是0.01，-0.01, 且 0.001 的选择可能会影响 single的 reconstruction
            maxT, minT = 0.01, -0.01
            nonmnfld_temp = 0.1 * torch.randn_like(pc_input) * self.local_sigma
            temp_gt = torch.clamp(nonmnfld_temp[:,:,2], minT, maxT)
            nonmnfld_temp[:,:,:2] = pc_input[:,:,:2]
            nonmnfld_temp[:,:,2] = temp_gt + pc_input[:,:,2]
            return nonmnfld_temp, temp_gt
