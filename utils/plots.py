...
def plot_surface(decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,save_npz,overwrite, points=None, with_points=False, latent=None, connected=False):
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        verts_array = surface["verts_array"]
        ...
        if (with_points):
            ...
            # fig1 = go.Figure(data=trace_pnts, layout=layout)
        else:
            fig1 = go.Figure(data=trace_surface, layout=layout)

        if (save_npz):
            np.savez(filename + '.npz', xyz = verts_array)
        if (save_html):
...
def plot_surface_valid(decoder,path,epoch, shapename, status,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,save_npz,overwrite, points=None, with_points=False, latent=None, connected=False):

    filename = '{0}/igr_{1}_{2}_{3}'.format(path, epoch, shapename,status)

    if (not os.path.exists(filename) or overwrite):

        if with_points:
            pnts_val = decoder(points)
            pnts_val = pnts_val.clone().detach()
            xyz_zdf = torch.cat((points,pnts_val),1)
            np.savez(filename + '_zdf.npz', valid_df = xyz_zdf.cpu().numpy())
    return
        #     pnts_val = pnts_val.cpu()
        #     points = points.cpu()
        #     caption = ["decoder : {0}".format(val.item()) for val in pnts_val.squeeze()]
...
def plot_surface_ani(iter, decoder,path,epoch, shapename,resolution,mc_value,is_uniform_grid,verbose,save_html,save_ply,save_npz,overwrite, points=None, with_points=False, latent=None, connected=False):

    filename = '{0}/igr_{1}_{2}_{3}'.format(path, epoch, shapename,iter)
